...
@cl.on_chat_start
async def start():
    await cl.Message(content="Welcome to Chat-GPT PDF Q&A!").send()
    files = None
    while not files:
        files = await cl.AskFileMessage(
            content="Upload text/PDF file to chat with it",
            accept=["text/plain", "application/pdf"],
            max_size_mb=chat_settings.MAX_SIZE_MB,
            timeout=chat_settings.TIMEOUT_SECS
        ).send()
    file = files[0]
    ...
    chain = RetrievalQAWithSourcesChain.from_chain_type(
        ...
        chain_type="stuff",
        retriever=doc_search.as_retriever(max_tokens_limit=chat_settings.MAX_TOKENS_LIMIT)
    )
